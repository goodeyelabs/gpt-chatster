                    ...
                    Object.keys(sessions[activeSession].messages).map((m:string, m_index:number) => {
                        ...
                        if (msg.author === 'server') {
                            return (
                                <div key={m_index} className='grid gap-3 w-[90%] place-self-start justify-start justify-items-start'>
                                    <div className='grid cursor-pointer rounded-[20px] bg-slate-200/40 dark:bg-zinc-800 px-6 py-2'>
                                        <p className='text-base text-slate-500 dark:text-slate-300 font-medium tracking-tight'>{msg.message || 'New message'}</p>
                                    </div>
                        ...
                        return (
                            <div key={m_index} className='grid gap-3 w-[90%] place-self-end justify-end justify-items-end'>
                                <div className='grid cursor-pointer rounded-[20px] bg-slate-400 dark:bg-slate-700 px-6 py-2'>
                                    <p className='text-base text-white dark:text-slate-200 font-medium tracking-tight'>{msg.message || 'New message'}</p>
                                </div>
